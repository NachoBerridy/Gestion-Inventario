...
import CrearNuevaOrden, {
  defaultConfOrder,
} from "../OrdenesDeCompra/CrearNuevaOrden";
...
export default function Articulos() {
  ...
  const [newAutoOrden, setNewAutoOrden] = useState<defaultConfOrder | null>(
    null
  );
  ...
  const getPaginationButtons = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      buttons.push(
        <button
          key={i}
          className={`p-2 rounded-md mx-1 shadow-sm transition duration-300 ${
  ...
  async function createAutoOrder(articuloId: number) {
    ...
    const autoOrder: defaultConfOrder = {
      ...
      orderQ: Math.round(
        responseInv.data?.articulo?.CalculosInventario?.loteOptimo
      ),
    };
