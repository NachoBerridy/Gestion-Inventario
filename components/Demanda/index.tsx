...
//   YAxis,
// } from "recharts";
import TableEST, { CyclesWithPeriods } from "./TableEstacionalidad";
import ModalBestOption from "./modalBestOption";
...
export default function Demanda() {
  ...
  const [cycles, setCycles] = useState<CyclesWithPeriods[]>([]);
  ...
  const [bestMetod, setBestMetod] = useState<string>("Mejor Metodo...");
  const [historicalDemand, setHistoricalDemand] = useState<SeparetedSales[]>([]);
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 p-4 w-full h-full">
      ...
      {openModal &&
        <ModalBestOption setShow={setOpenModal} setBestMetod={setBestMetod} historicalDemand={historicalDemand} />
      }
    </div>
  );
}